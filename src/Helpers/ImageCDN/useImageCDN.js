...
export function getProjectImagesCDN (projectName) {
  const windowSize = getWindowWidth()
  return axios.get(`${CDN_DEFS.CDN_DOMAIN_PREFIX}${CDN_DEFS.REACT_APP_CDN_CLOUD_NAME}/image/list/${windowSize}-${projectName}.json`)
}

export function getGalleryImagesCDN (galleryName) {
  const windowSize = getWindowWidth()
